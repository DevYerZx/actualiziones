...
// Interceptar
console.log = (...args) => { _pushConsoleLine("LOG", args); __log(...args); };
console.error = (...args) => { _pushConsoleLine("ERROR", args); __error(...args); };
console.warn = (...args) => { _pushConsoleLine("WARN", args); __warn(...args); };
        ...
        for (const a of archivos) {
            ...
            try {
                ...
                for (const n of nombres) {
                    comandos.set(n.toLowerCase(), cmd);
                }
...
async function iniciarBot() {
    ...
    sock.ev.on('messages.upsert', async ({ messages }) => {
        ...
        const numeroSender = sender.split('@')[0];
        ...
        const esOwner = owners.includes(numeroSender);
